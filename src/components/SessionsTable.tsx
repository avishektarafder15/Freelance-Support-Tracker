...
import { Search, Filter, Download, ExternalLink, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export const SessionsTable: React.FC<SessionsTableProps> = ({ sessions }) => {
  ...
  const filteredSessions = sessions.filter(session => 
    ...
    session.platformType.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col h-full">
      ...
      <div className="p-4 border-b border-gray-100 bg-gray-50/50 flex flex-wrap items-center justify-between gap-4">
        <div className="relative flex-1 min-w-[300px]">
          ...
          <input
            type="text"
            placeholder="Search sessions by name, email, mentor, or platform..."
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-left border-collapse min-w-[1800px]">
          <thead className="sticky top-0 z-10">
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Platform Type</th>
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm">
                    <span className={`px-2 py-1 rounded-md text-xs font-medium ${
                      session.platformType === 'LinkedIn' ? 'bg-blue-50 text-blue-700' :
                      session.platformType === 'Upwork' ? 'bg-green-50 text-green-700' :
                  ...
                  <td className="px-4 py-3 text-sm">
                    {session.googleMeetUrl ? (
                      ...
                      >
                        ...
                        {session.googleMeetUrl.replace('https://', '')}
                      </a>
                    ) : '-'}
                  </td>
                  <td className="px-4 py-3 text-sm">
        ...
        {filteredSessions.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            <p className="text-lg font-medium">No sessions found matching your search.</p>
          </div>
        )}
      </div>
      ...
      <div className="p-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between text-xs text-gray-500">
        <div>Total Sessions: <span className="font-bold text-gray-700">{filteredSessions.length}</span></div>
